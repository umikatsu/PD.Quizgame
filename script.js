document.addEventListener("DOMContentLoaded", () => {
    ...
    // quizImageはimageContainer内で使用するため、ここでは直接使用を避ける
    // const quizImage = document.getElementById('quiz-image'); 
    // 💡 修正: HTMLに静的追加されたIDを信頼して取得する
    const imageContainer = document.getElementById('quiz-image-container');

    
const quizzes = [
    {question: "水害時避難する時に履くべき靴はどちらか？", choices: ["長靴", "スニーカー", "サンダル", "ハイヒール"], answer: "スニーカー", explanation: "長靴は浸水時に水が内部に入り、移動しづらくなってしまうため不適切です。ひもで結べて、足底がギザギザした滑りずらいスニーカーが避難時には適切です。", image: null},
    {question: "金沢市が作成している水害ハザードマップに書かれていないものは次のうちどれか？", choices: ["浸水想定区域", "警戒レベルごとに行うべき行動", "土砂災害想定区域", "高潮浸水想定区域図"], answer: "高潮浸水想定区域図", explanation: "水害ハザードマップは大雨により河川が氾濫した場合を想定したものなので、高潮に関する想定区域図はありません。", image: null},
    {question: "ハザードマップに記されている洪水の被害はどれくらいの規模を想定して作られているか？", choices: ["10年に1回", "100年に1回", "1000年以上に1回", "5000年以上に1回"], answer: "1000年以上に1回", explanation: "ハザードマップには１０００年以上に１回起こると考えられる洪水の被害が記されています。", image: null},
    {question: "災害前に用意するべきものとして間違っているものはどれか？", choices: ["非常用持ち出し袋の準備", "備蓄品の購入", "罹災（りさい）証明書の申請", "家族との連絡方法の確認"], answer: "罹災（りさい）証明書の申請", explanation: "罹災証明書の申請は、災害により家屋などに被害が出たことを証明する書類であり、災害後に自治体に申請する物なので間違いです。", image: null},
    {question: "災害時の非常食として適していないものはどれか？", choices: ["缶詰", "ビスケット", "カップ麺", "栄養補助食品"], answer: "カップ麺", explanation: "カップ麺はお湯を沸かして食べる必要があり、災害時はお湯を沸かすのはもちろん、水すら十分に入手することが難しい恐れがあるため不適です。", image: null},
    {question: "警戒レベルは5段階あるが、全員が避難するべき警戒レベルはどれか？", choices: ["レベル3", "レベル4", "レベル5", "レベル1"], answer: "レベル4", explanation: "警戒レベル5は避難行動が既に行えない状態を示しているため、レベル4のタイミングで避難行動を行わなければなりません。高齢者などの避難行動に時間がかかる人は警戒レベル3のタイミングで行う必要があります。", image: null},
    {question: "水圧により成人男性が扉を開けられなくなるのはどれくらいの水位からか？", choices: ["10cm", "30cm", "50cm", "80cm"], answer: "50cm", explanation: "20～30cmでドアにかかる水圧は数十キロになり女性や高齢者では開けられなくなり、50cmを超えると100キロ以上になり男性でも開けることは不可能になります。", image: null},
    ...
    // 💡 ブレーカー問題: 上に「スクリーンショット 2025-11-14 150140.png」、下に「bureka.png」
    {question: "電気を復旧させるときの手順として正しい並び替えはどれか？ (画像参照)", 
      ...
      answer: "4→2→3→1", 
      explanation: "正解は 4.ブレーカーが全てOFFになっているか確認 → 2.アンペアブレーカーをON → 3.漏電遮断器をON → 1.安全ブレーカーを一つずつON の順序です。手順は画像を参照してください。", 
      image: ["スクリーンショット 2025-11-14 150140.png", "bureka.png"] 
    }
];
    ...
    /**
     * 知識クイズの表示ロジック
     * 💡 修正: 複数の画像を imageContainer に挿入するように調整
     */
    function displayQuiz() {
        ...
        // 💡 HTMLに静的追加された imageContainer を取得し、中身をクリア
        const currentImageContainer = document.getElementById('quiz-image-container');
        currentImageContainer.innerHTML = ''; 

        // 画像処理
        if (currentQuiz.image) {
            let images = Array.isArray(currentQuiz.image) ? currentQuiz.image : [currentQuiz.image];

            images.forEach((src, i) => {
                let img = document.createElement('img');
                img.src = src;
                img.alt = 'クイズ画像';
                img.classList.add('quiz-visual');
                
                // 1枚目のみIDを `quiz-image` に設定 (HTMLの元の要素IDを保持)
                if (i === 0) {
                    img.id = 'quiz-image'; 
                }
        ...
        // 選択肢をシャッフルし、インデックス番号を付与しない
        shuffledChoices.forEach((choice, i) => { 
            const button = document.createElement('button');
            
            // 選択肢のテキストに、自動でインデックス番号を付与しない
            ...
            button.addEventListener('click', () => {
                // 選択肢のテキスト（例: "スニーカー" または "4→2→3→1"）を渡す
    ...
    function checkAnswer(selectedButton, selectedChoice, correctAnswer) {
        ...
        // selectedChoice (選択肢の元のテキスト) と correctAnswer (正解のテキスト) を直接比較
        if (selectedChoice === correctAnswer) { 
            ...
            // 正解ボタンを探すとき、ボタンのtextContentを直接比較する
            buttons.forEach(btn => {
                // ボタンのtextContentとcurrentQuiz.answerを直接比較する
