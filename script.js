...
const quizzes = [
    ...
    {question: "電気を復旧させるときの手順として正しい並び替えはどれか？ (画像参照)", 
      choices: [
          "1.安全ブレーカーを一つずつONにする", // 💡 再度、番号付きの元のテキストに戻しました
          "2.アンペアブレーカーを入れる", 
          "3.漏電遮断器をONにする", 
    ...
    function displayQuiz() {
        ...
        // ★ 修正済み: 選択肢をシャッフルし、インデックス番号を付与しない
        shuffledChoices.forEach((choice, i) => { 
            const button = document.createElement('button');
            
            // 修正: 選択肢のテキストに、自動でインデックス番号を付与しない
    ...
    /**
     * 知識クイズの回答チェックロジック
     * 正解ボタンのハイライト処理で、番号付き/番号なしの選択肢に対応するため、
     * ボタンのテキストから先頭の番号を削除してから比較するロジックを復活させました。
     */
    function checkAnswer(selectedButton, selectedChoice, correctAnswer) {
        ...
        // selectedChoice (選択肢の元のテキスト) と correctAnswer (正解のテキスト) を比較
        if (selectedChoice === correctAnswer) { 
            ...
            // ★ 修正済み: 正解ボタンを探すとき、ボタンのtextContentから番号を削除してから比較するロジックを復活
            // これにより、ブレーカーのクイズ（番号付きテキスト）と他のクイズ（番号なしテキスト）の両方に対応
            buttons.forEach(btn => {
