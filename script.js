document.addEventListener("DOMContentLoaded", () => {
    ...
    const quizzes = [
        ...
        {question: "電気を復旧させるときの手順として正しい並び替えはどれか？ (画像参照)", choices: ["4→2→3→1", "2→3→1→4", "4→3→2→1", "2→4→3→1"], answer: "4→2→3→1", explanation: "正解は 4.ブレーカーが全てOFFになっているか確認 → 2.アンペアブレーカーをON → 3.漏電遮断器をON → 1.安全ブレーカーを一つずつON の順序です。", image: "kuizugamedetsukauyatsu.png"}
    ];
    ...
    function displayQuiz() {
        ...
        // ★ 修正点1: 選択肢をシャッフルし、表示時にインデックス（番号）を付与する
        shuffledChoices.forEach((choice, i) => { 
            const button = document.createElement('button');
            
            // 選択肢のテキストの前に、(インデックス + 1) とドットとスペースを追加
            ...
            button.addEventListener('click', () => {
                // 回答チェック時には、データにある番号なしの純粋な選択肢テキストを渡す
    ...
    function checkAnswer(selectedButton, selectedChoice, correctAnswer) {
        ...
        // selectedChoice (番号なし) と correctAnswer (番号なし) を比較
        if (selectedChoice === correctAnswer) { 
            ...
            // ★ 修正点2: 正解ボタンを探すとき、ボタンのテキストから番号を削除してから比較する
            buttons.forEach(btn => {
                // ボタンのtextContentから「1. 」「2. 」といった接頭辞を削除する
                const buttonTextWithoutPrefix = btn.textContent.replace(/^\d+\.\s*/, ''); 
                if (buttonTextWithoutPrefix === currentQuiz.answer) {
