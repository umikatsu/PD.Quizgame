document.addEventListener("DOMContentLoaded", () => {
    ...
    // 知識クイズ問題データ
    const quizzes = [
        {question: "水害時避難する時に履くべき靴はどちらか？", choices: ["1.長靴", "2.スニーカー", "3.サンダル", "4.ハイヒール"], answer: "2.スニーカー", explanation: "長靴は浸水時に水が内部に入り、移動しづらくなってしまうため不適切です。ひもで結べて、足底がギザギザした滑りずらいスニーカーが避難時には適切です。", image: null},
        {question: "金沢市が作成している水害ハザードマップに書かれていないものは次のうちどれか？", choices: ["1.浸水想定区域", "2.警戒レベルごとに行うべき行動", "3.土砂災害想定区域", "4.高潮浸水想定区域図"], answer: "4.高潮浸水想定区域図", explanation: "水害ハザードマップは大雨により河川が氾濫した場合を想定したものなので、高潮に関する想定区域図はありません。", image: null},
        {question: "ハザードマップに記されている洪水の被害はどれくらいの規模を想定して作られているか？", choices: ["1.10年に1回", "2.100年に1回", "3.1000年以上に1回", "4.5000年以上に1回"], answer: "3.1000年以上に1回", explanation: "ハザードマップには１０００年以上に１回起こると考えられる洪水の被害が記されています。", image: null},
        {question: "災害前に用意するべきものとして間違っているものはどれか？", choices: ["1.非常用持ち出し袋の準備", "2.備蓄品の購入", "3.罹災（りさい）証明書の申請", "4.家族との連絡方法の確認"], answer: "3.罹災（りさい）証明書の申請", explanation: "罹災証明書の申請は、災害により家屋などに被害が出たことを証明する書類であり、災害後に自治体に申請する物なので間違いです。", image: null},
        {question: "災害時の非常食として適していないものはどれか？", choices: ["1.缶詰", "2.ビスケット", "3.カップ麺", "4.栄養補助食品"], answer: "3.カップ麺", explanation: "カップ麺はお湯を沸かして食べる必要があり、災害時はお湯を沸かすのはもちろん、水すら十分に入手することが難しい恐れがあるため不適です。", image: null},
        {question: "警戒レベルは5段階あるが、全員が避難するべき警戒レベルはどれか？", choices: ["1.レベル3", "2.レベル4", "3.レベル5", "4.レベル1"], answer: "2.レベル4", explanation: "警戒レベル5は避難行動が既に行えない状態を示しているため、レベル4のタイミングで避難行動を行わなければなりません。高齢者などの避難行動に時間がかかる人は警戒レベル3のタイミングで行う必要があります。", image: null},
        {question: "水圧により成人男性が扉を開けられなくなるのはどれくらいの水位からか？", choices: ["1.10cm", "2.30cm", "3.50cm", "4.80cm"], answer: "3.50cm", explanation: "20～30cmでドアにかかる水圧は数十キロになり女性や高齢者では開けられなくなり、50cmを超えると100キロ以上になり男性でも開けることは不可能になります。", image: null},
        {question: "水害発生時、山へ逃げるために車を使い避難行動することは〇か×か？", choices: ["1.〇（適切）", "2.×（不適切）"], answer: "2.×（不適切）", explanation: "災害時車を利用すると渋滞や事故の元になり、緊急車両の通行が困難になる恐れがあります。また、水害では30cm浸かる状態でエンジンが停止してしまい避難行動すらできなくなるため不適切です。", image: null},
        {question: "家屋が被災した時にすぐに確認すべき、火災・爆発の危険がある重要な項目はどれか？", choices: ["1.被害状況を写真で記録すること", "2.ブレーカーとガスの安全確認", "3.避難所への場所の確認", "4.近隣住民の安否確認"], answer: "2.ブレーカーとガスの安全確認", explanation: "特にブレーカー（電気）やガスに異常があると発火や爆発の恐れがあるため、迅速に安全確認を行う必要があります。", image: null},
        {question: "電気を復旧させるときの手順として正しい並び替えはどれか？ (画像参照)", choices: ["1. 4→2→3→1", "2. 2→3→1→4", "3. 4→3→2→1", "4. 2→4→3→1"], answer: "1. 4→2→3→1", explanation: "正解は 4.ブレーカーが全てOFFになっているか確認 → 2.アンペアブレーカーをON → 3.漏電遮断器をON → 1.安全ブレーカーを一つずつON の順序です。", image: "スクリーンショット 2025-10-31 142014.png"}
    ];
    ...
    const pQuestions = [
        ...
        {question: "4. 避難時に持っていくものを選ぶ際、何を優先しますか？", options: [{ text: "食料、水、医薬品など、生命維持に必須なもの", score: 3 }, { text: "貴重品や連絡手段（スマホなど）", score: 2 }, { text: "服やお気に入りのものなど、気分が落ち着くもの", score: 1 }]},
    ...
    function displayQuiz() {
        ...
        // 画像参照 (GitHub Pages のベース URL は HTMLの <base> タグによって処理されます)
    ...
    function showPResult() {
        ...
        if (result) {
            pResultText.innerHTML = result.text + `<p><em>（合計スコア: ${totalPScore}点）</em></p>`;
        } else {
