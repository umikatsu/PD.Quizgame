document.addEventListener("DOMContentLoaded", () => {
    ...
    const quizzes = [
        ...
        {question: "家屋が被災した時にすぐに確認すべき、火災・爆発の危険がある重要な項目はどれか？", choices: ["1.被害状況を写真で記録すること", "2.ブレーカーとガスの安全確認", "3.避難所への場所の確認", "4.近隣住民の安否確認"], answer: "2.ブレーカーとガスの安全確認", explanation: "特にブレーカー（電気）やガスに異常があると発火や爆発の恐れがあるため、迅速に安全確認を行う必要があります。", image: null}, // ★修正済み
        {question: "電気を復旧させるときの手順として正しい並び替えはどれか？ (画像参照)", choices: ["1. 4→2→3→1", "2. 2→3→1→4", "3. 4→3→2→1", "4. 2→4→3→1"], answer: "1. 4→2→3→1", explanation: "正解は 4.ブレーカーが全てOFFになっているか確認 → 2.アンペアブレーカーをON → 3.漏電遮断器をON → 1.安全ブレーカーを一つずつON の順序です。", image: "kuizugamedetsukauyatsu.png"}
    ];
